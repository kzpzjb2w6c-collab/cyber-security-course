...
## c) Eve and Mallory.
In many crypto stories, Eve is a passive eavesdropper, listening on the wire. Mallory malliciously modifies the messages. Explain how PGP protects against Mallory and Eve. Be specific what features, which use of keys and which flags in the command are related to this protection. 
- In PGP when a command `gpg --encrypt --recipient (receiver's email) --armor message.txt` is used, PGP encrypts the message with the receiver's public key. This means that for Eve, they can only see cipher text, which is not readable. They can't see the message without the receiver's private key, which is used to decrypt from cipher text to plain text.
- A command line `gpg --encrypt --sign --recipient (receiver's email) --armor message.txt` and it's `sign` command creates a digital signature using my public key. So if Mallory changes the contents of the message the signature check fails as it checks the public key on the receiver's side. The signature is a bit of like a wax seal on an envolope with a specific symbol to tell the receiver side that the seal hasn't been broken and the content of the envelope has not been tampered.

## d) Password management.
I have experience in using Apple's *Passwods* -password management system. It is integrated well into Apple's devices, where each time I register to a website it suggests a strong password that I can then store into Passwords app that I can access with a password or with biometric authentication. It also tells me if a password has been found in a leak so I can change the password. 

The benefit of using a password management system is that if one password leaks from the services where I have registered, it decreases the risk of having all of the services vulnerable. If a person used the same password for all the services and it leaks, there is a high risk that the person not using a password management system does not remember all the services where they have been registered with the same password, which increases the risk of having personal information stolen. Without a password management system, it's also difficult to get notified if the password has been found in leaks.
